/**
 * Virtualized Candidate Table
 * Uses virtual scrolling for efficient rendering of large candidate lists
 */
...
interface VirtualizedCandidateTableProps {
...
// Helper functions (same as CandidateTable)
...
const VirtualCandidateRow = memo(({ 
  candidate, 
  onRowClick,
  style
}: { 
  ...
  return (
    <TableRow
      style={style}
      className="cursor-pointer hover:bg-muted/50 absolute left-0 right-0"
      onClick={handleClick}
      tabIndex={0}
...
VirtualCandidateRow.displayName = 'VirtualCandidateRow'
...
export const VirtualizedCandidateTable = memo(({ 
  candidates, 
  sortField, 
  sortOrder, 
  onSort, 
  ...
  rowHeight = 73,
  overscan = 5
}: VirtualizedCandidateTableProps) => {
  ...
  const totalHeight = sortedCandidates.length * rowHeight
  ...
  const visibleCandidates = useMemo(() => {
    return sortedCandidates.slice(startIndex, endIndex + 1).map((candidate, i) => ({
      candidate,
      index: startIndex + i
    }))
  }, [sortedCandidates, startIndex, endIndex])
...
VirtualizedCandidateTable.displayName = 'VirtualizedCandidateTable'
